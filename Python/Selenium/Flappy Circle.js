...
\f0\fs24 \cf0 var updateInterval = false,\
    keyListener = false;\
var isPlaying = false;\
...
update = function() \{\
    ...
    if (hideCircle) \{\
        ...
        currentAlpha = Math.max(0, currentAlpha - .05)\
    \}\
    context.drawImage(circle_l, PosNow[0] - 52, PosNow[1] - 79, 52, 157);\
    if (errorAlphaA > 0 && !hideCircle) \{\
        context.globalAlpha = Math.max(.1, Math.abs(Math.sin(errorAlphaA))) * .7;\
    ...
    context.drawImage(circle_r, PosNow[0],\
        PosNow[1] - 79, 52, 157);\
    if (errorAlphaA > 0 && !hideCircle) \{\
        context.globalAlpha = Math.max(.1, Math.abs(Math.sin(errorAlphaA))) * .7;\
    ...
    if (isPlaying) Vy = Vy + .2;\
    else if (errorAlphaA == 0) \{\
        Vy = Math.sin(readySin) * .4;\
        readySin += .06\
    \}\
